$(document).ready(function() {
  ...
  const animateOut = jQLink => {
    ...
    //animated changes
    solid.animate(
      { height: "100%", "max-width": "none", width: "3000%" },
      { duration: animationLength / 6, queue: false }
    );
    footer.animate(
      { height: "10px", "margin-top": "105px" },
      { duration: animationLength / 4, queue: false }
    );
    all.animate(
      { left: "-50%", top: "-50%" },
    ...
    main.animate(
      ...
      { duration: animationLength / 2, queue: false }
    );
  ...
  const animateIn = jQLink => {
    ...
    solid.animate(
      { height: "0%", "max-width": "none", width: "100%" },
      { duration: animationLength / 4, queue: false }
    );
    footer.animate(
      { height: "90px", "margin-top": "25px" },
      { duration: animationLength / 4, queue: false }
    );
      ...
      if (data === "lego") {
        ...
        const legoSlideshow = () => {
          ...
          setInterval(function() {
            ...
            $("#current-image").attr(
              "src",
              `http://farm${currentPhoto.farm}.staticflickr.com/${
                currentPhoto.server
        ...
        $.ajax({
          url:
            "https://api.flickr.com/services/rest/?api_key=9838075b647ec1a2393ba502cb82c148&gallery_id=72157677715358868&method=flickr.galleries.getPhotos&format=json&nojsoncallback=1",
          method: "GET"
